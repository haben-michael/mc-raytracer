...
R3Cylinder::
R3Cylinder(const R3Point& center, R3Vector axis, double radius, double height)
  : center(center),
	axis(axis),
    radius(radius),
    height(height)
...
BBox(void) const
{
  ...
  bbox.Union(center - 0.5 * height * axis + radius * R3negx_vector + radius * axis);
  bbox.Union(center - 0.5 * height * axis + radius * R3posx_vector + radius * axis);
  return bbox;
}
